(ns org.candelbio.pret.db.transact
  (:require [datomic.api :as d]
            ...
            [clojure.edn :as edn]
            [org.candelbio.pret.db.import-coordination :as ic]
            [org.candelbio.pret.db.common :refer [retryable?]]
...
(defn strip-annotations [tx-elem]
  (if (map? tx-elem)
    [(dissoc tx-elem :pret/annotations)]
    [tx-elem]))

(defn flatten-annotations [tx-elem]
  (if-not (and (map? tx-elem)
               (:pret/annotations tx-elem))
    [tx-elem]
    (let [tempid (util.uuid/random)]
      [(-> tx-elem
           (dissoc :pret/annotations)
           (assoc :db/id tempid))
       (assoc (:pret/annotations tx-elem) :pret.annotation/entity tempid)])))
...
(defn async-transact-w-retry
  ...
  (let [{:keys [skip-annotations]} opts
      ...
      (if-not (::anom/category res)
        ...
        (let [file (->> tx-data
                        (map :pret.annotation/filename)
                        (remove nil?)
                        first)
              line-numbers (->> tx-data
                                (map :pret.annotation/line-number)
                                (remove nil?))]
         (merge res
                (when (and file (seq line-numbers))
                  {:pret.annotation/filename file
                   :pret.annotation/start-line (apply min line-numbers)
                   :pret.annotation/end-line (apply max line-numbers)}))))))
...
(comment
  ...
  (in-ns 'org.candelbio.pret.db.transact)
  (def path "/Users/bkamphaus/scratch/template-diff-clean/tx-data/import-job.edn")
  (def path "/Users/bkamphaus/scratch/template-diff-clean/tx-data/pret-ref-0001-gc_ref_cnv_1.tsv-6b4a98ae-6fe9-5b7e-b5ee-fb197dabe7cd.edn")
  (def path "/Users/bkamphaus/scratch/template-diff-clean/tx-data/pret-ref-0001-gc_ref_cnv_3.tsv-34ea5f0e-f617-5cfb-b282-05919fb436a1.edn")
