(ns org.candelbio.pret.validation.post-import.util
...
(defn- get-annotations-for-entity
  "Get pret.annotation information for an entity e (in map form). Returns
  ...
  (let [eid (:db/id e)]
    (d/q '[:find [?filename ?line-number]
           :in $ ?eid
           :where
           [?a :pret.annotation/entity ?eid]
           [?a :pret.annotation/filename ?filename]
           [?a :pret.annotation/line-number ?line-number]]
         db eid)))
...
(defn reference-data-entities
  ...
  (let [xf (comp
    ...
    (sequence
      xf
      (d/datoms db :vaet [:import/name import-name] :pret.import/most-recent))))
...
(defn entity-has-valid-uid?
  ...
  (let [uid (get-uid-from-entity metamodel e)]
    (if-not (and (some? uid)
      ...
      (merge-with concat
        e
        {:pret.validation/errors [(str "---- "(format-entity db e))]})
      e)))

(defn dangling-ref?
  "If an entity does not have attributes other than UID, it was created by reference only."
  [db metamodel entity]
  (let [e (dissoc entity :db/id :pret.validation/errors)
    ...
    (if (seq other-keys)
      ...
      (merge-with concat entity
                  {:pret.validation/errors [(str "---- The entity " (format-entity db entity)
                                                 " was referenced from somewhere but not defined anywhere.")]}))))

(defn attr-namespaces-match?
  "If non pret/db attribute namespaces don't match, this is a mistake."
  [db entity]
  (let [e (dissoc entity :db/id :pret.validation/errors)
        attr-keys (keys e)
...
(defn- spec-validation
  ...
  (let [e (dissoc entity :db/id :pret.validation/errors)
    ...
    (if (and
          (not= kind "pret.annotation") ; Skip annotation entities
          (not (s/valid? spec (dissoc e :pret.validation/errors)))) ;; Skip previous errors
      (merge-with concat entity
        {:pret.validation/errors
