(ns org.candelbio.pret.validation.specs
...
(s/def :pret/sample (s/schema [:sample/id
                               :sample/uid
                               :sample/subject]))
(s/def ::sample (s/select :pret/sample [*]))

(s/def :pret/dataset (s/schema [:dataset/name
                                :dataset/samples
                                :dataset/assays
                                :dataset/timepoints
                                :dataset/subjects]))
(s/def ::dataset (s/select :pret/dataset [*]))

(s/def :pret/assay (s/schema [:assay/uid
                              :assay/name
                              :assay/measurement-sets
                              :assay/technology]))
(s/def ::assay (s/select :pret/assay [*]))


(s/def :pret/measurement-set-measurements (s/schema [:measurement-set/uid
                                                     :measurement-set/name
                                                     :measurement-set/measurements]))
(s/def :pret/measurement-set-matrix (s/schema [:measurement-set/uid
                                               :measurement-set/name
                                               :measurement-set/measurement-matrices]))
(s/def ::measurement-set
  (s/or ::ms-measurements (s/select :pret/measurement-set-measurements [*])
        ::ms-matrices (s/select :pret/measurement-set-matrix [*])))

(s/def :pret/clinical-observation-set (s/schema [:clinical-observation-set/uid
...
(s/def ::clinical-observation-set (s/merge
                                    (s/select :pret/clinical-observation-set [:clinical-observation-set/name
                                                                              :clinical-observation-set/uid])
                                    (s/or :clinical-observation-set/adverse-events (s/select :pret/clinical-observation-set [:clinical-observation-set/adverse-events])
                                          :clinical-observation-set/clinical-observations (s/select :pret/clinical-observation-set [:clinical-observation-set/clinical-observations]))))

(s/def :pret/clinical-observation (s/schema [:clinical-observation/id
                                             :clinical-observation/uid
                                             :clinical-observation/subject
                                             :clinical-observation/timepoint]))
(s/def ::clinical-observation (s/select :pret/clinical-observation [*]))

(s/def :pret/adverse-event (s/schema [:adverse-event/id
                                      :adverse-event/uid
                                      :adverse-event/subject
                                      :adverse-event/meddra-adverse-event]))
(s/def ::adverse-event (s/select :pret/adverse-event [*]))

(s/def :pret/timepoint (s/schema [:timepoint/id
                                  :timepoint/uid
                                  :timepoint/type
                                  :timepoint/relative-order
                                  :timepoint/treatment-regimen]))
(s/def ::timepoint
  (s/merge (s/select :pret/timepoint [:timepoint/id
                                      :timepoint/uid
                                      :timepoint/type])
...
(s/def :pret/genomic-coordinate (s/schema [:genomic-coordinate/id
...
(s/def ::genomic-coordinate
  (s/and
    (s/select :pret/genomic-coordinate [*])
    ::matching-genomic-coordinate-id))

(s/def :pret/variant (s/schema [:variant/id
...
(s/def ::variant
  (s/and
    (s/select :pret/variant [*])
    ::matching-variant-id))

(s/def :pret/gene (s/schema [:gene/hgnc-symbol
                             :gene/hgnc-id
                             :gene/hgnc-name]))
(s/def ::gene (s/select :pret/gene [*]))

(s/def :pret/protein (s/schema [:protein/uniprot-name
                                :protein/uniprot-accessions
                                :protein/preferred-name]))
(s/def ::protein (s/select :pret/protein [*]))

(s/def :pret/subject (s/schema [:subject/id :subject/uid]))
(s/def ::subject (s/select :pret/subject [*]))

(s/def :pret/treatment-regimen (s/schema [:treatment-regimen/name
                                          :treatment-regimen/uid]))
(s/def ::treatment-regimen (s/select :pret/treatment-regimen [*]))


(s/def :pret/cell-population (s/schema [:cell-population/name
                                        :cell-population/cell-type
                                        :cell-population/from-clustering]))
(s/def ::cell-population
  (s/merge (s/select :pret/cell-population [:cell-population/name :cell-population/from-clustering])
           (s/or :cell-population-from-clustering #(:cell-population/from-clustering %)
                 :cell-population/from-gating (s/and #(not (:cell-population/from-clustering %))
                                                     (s/select :pret/cell-population [:cell-population/cell-type])))))

(s/def :pret/tcr (s/schema [:tcr/id
                            :tcr/alpha-1
                            :tcr/alpha-2
                            :tcr/beta]))

(s/def ::tcr
  (s/merge (s/select :pret/tcr [:tcr/id])
           (s/or :tcr-alpha (s/select :pret/tcr [:tcr/alpha-1])
                 :tcr-beta (s/select :pret/tcr [:tcr/beta]))))


(s/def :pret/therapy (s/schema [:therapy/id
                                :therapy/uid
                                :therapy/treatment-regimen
                                :therapy/order]))
(s/def ::therapy (s/select :pret/therapy [*]))

(s/def :pret/cnv (s/schema [:cnv/genomic-coordinates]))
(s/def ::cnv (s/select :pret/cnv [*]))


(s/def :pret/measurement (s/schema [:measurement/id
                                    :measurement/uid
                                    :measurement/sample]))

(s/def :pret/import (s/schema [:import/name :import/user]))
(s/def ::import (s/select :pret/import [*]))

(s/def :pret/drug-regimen (s/schema [:drug-regimen/uid
                                     :drug-regimen/drug]))
(s/def ::drug-regimen (s/select :pret/drug-regimen [*]))

(s/def :pret/clinical-trial (s/schema [:clinical-trial/nct-number]))
(s/def ::clinical-trial (s/select :pret/clinical-trial [*]))

(s/def :pret/atac-peak (s/schema [:atac-peak/name
                                  :atac-peak/genomic-coordinates
                                  :atac-peak/jointly-called
                                  :atac-peak/fixed-width]))

(s/def ::atac-peak (s/select :pret/atac-peak [*]))

(s/def :pret/otu (s/schema [:otu/id
                            :otu/kingdom
                            :otu/phylum
                            :otu/class
                            :otu/order
...
(s/def ::otu (s/select :pret/otu [:otu/id :otu/kingdom]))


(s/def :pret/meddra-disease (s/schema [:meddra-disease/preferred-name
                                       :meddra-disease/synonyms]))
(s/def ::meddra-disease (s/select :pret/meddra-disease [*]))

(s/def :pret/gdc-anatomic-site (s/schema [:gdc-anatomic-site/name]))
(s/def ::gdc-anatomic-site (s/select :pret/gdc-anatomic-site [*]))

(s/def :pret/gene-product (s/schema [:gene-product/id
                                     :gene-product/gene]))
(s/def ::gene-product (s/select :pret/gene-product [*]))

(s/def :pret/epitope (s/schema [:epitope/id
                                :epitope/protein]))
(s/def ::epitope (s/select :pret/epitope [*]))

(s/def :pret/single-cell (s/schema [:single-cell/id]))
(s/def ::single-cell (s/select :pret/single-cell [*]))

(s/def :pret/nanostring-signature (s/schema [:nanostring-signature/name
                                             :nanostring-signature/gene-weights]))
(s/def ::nanostring-signature (s/select :pret/nanostring-signature [*]))


(s/def :pret/study-day (s/schema [:study-day/id
                                  :study-day/uid
                                  :study-day/day
                                  :study-day/reference-event]))
(s/def ::study-day (s/select :pret/study-day [*]))
...
(s/def ::measurement
  (s/merge (s/select :pret/measurement [*])
