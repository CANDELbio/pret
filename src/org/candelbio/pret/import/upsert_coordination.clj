(ns org.candelbio.pret.import.upsert-coordination
...
(def ignored-entity-keys #{:import/import :db/id :import/txn-id :pret/annotations})
  ...
  (when-let [ent-keys (seq (remove ignored-entity-keys (keys ent)))]
    (when-let [unique-attr (first (filter (partial metamodel/unique? schema) ent-keys))]
      (let [unique-val (get ent unique-attr)]
        (assoc ent :pret/identifying-avpair [unique-attr unique-val])))))

(defn- remove-pret-keys
  [m]
  (into {} (remove #(= (namespace (key %)) "pret") m)))
...
(defn check-single-entity-for-upsert
  ...
  (let [filtered-ent (remove-pret-keys tx-ent)
...
(defn check-tx-for-upserts
  ...
  (let [av-pairs (keep (partial unique-av-pair schema) tx)
        indexed-av-pairs (map-indexed #(conj (:pret/identifying-avpair %2) %1) av-pairs)
