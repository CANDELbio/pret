(ns org.candelbio.pret.import.engine.parse.matrix-file
...
(defn parse-matrix-file
  ...
  Args:
  :indexed-by A map of colnames to pret schema attributes, e.g. {'colname' :measurement/value}
              ...
              (if-not this-row
                (merge (into {} (for [[key-by-pos index-vals] index-sets]
                       ...
                       (when target
                         {target non-ind-cols})
                       (when sparse?
                         {:pret.matrix/sparse-value-column (only non-ind-cols
