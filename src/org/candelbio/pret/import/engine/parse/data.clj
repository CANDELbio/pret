(ns org.candelbio.pret.import.engine.parse.data
  (:require [org.candelbio.pret.util.collection :as collection]
...
(defn- ctx-leaf->ctx-id-lookup
  ...
  (let [kw-only-ctx (filter keyword? (:pret/ns-node-ctx ctx-node))]
...
(defn resolve-uid
  ...
  (let [node-ctx (:pret/ns-node-ctx node)
...
(defn uid-attr-val
  ...
  (let [node-kind (:pret/node-kind node)]
    ;; remove pret scoped map annotations, e.g. reverse attr, glob context, etc.
    (when-not (= "pret" (namespace (last (filter keyword? (:pret/ns-node-ctx node)))))
  ...
  ([parsed-cfg schema mapping job attribute-name raw-value dt na-val-set rev-ref]
   ...
   (let [resolved-value (if-not job
     ...
     (if-let [ref (metamodel/kind-ref? schema attribute-name)]
       (if (= resolved-value :pret/missing-value)
         resolved-value
...
(defn- reverse-ref-uid-map
  "For entities that will contain reverse-reference's in the contextual path
   ...
   of the directive."
  [job entity]
  (when-let [rev (:pret/reverse job)]
    (let [ent-attr (:pret/rev-variable rev)]
...
(defn create-reverse-reference
  "If this job includes a :pret/reverse key, generate the appropriate reverse reference"
  [parsed-cfg schema mapping job raw-entity na-val-set]
  (when (:pret/reverse job)
    (let [rev-attr (:pret/rev-attr (:pret/reverse job))
          raw-value (get raw-entity (:pret/rev-variable (:pret/reverse job)))
...
(defn- tidy-entry?
  "Determines if a particular value is a tidy data att/val pair (ignores pret
  keywords, already resolved values)."
  [[k v]]
  (and
    (not (str/starts-with? (namespace k) "pret"))
    (string? v)))
  ...
  (->> job
       ...
       (keep
        (fn [[k v]]
          (cond
            (tidy-entry? [k v]) [k v]
            (= :pret/reverse k) [(:pret/rev-attr v)
                                 (:pret/rev-variable v)])))
       (into {})))
...
(defn extract-molten-data
  ...
  [parsed-cfg schema mapping job raw-entity na-val-set]
  (when (molten? job)
    (let [column-name (get raw-entity (:pret/variable job))
          ;; manual get chain instead of get-in is perf optimization
          attribute (get (get job :pret/variables) column-name)
          value (get raw-entity (:pret/value job))]
      (when-not (and column-name attribute)
        (throw (ex-info (str "Specified column name or attribute mismatch between config and file: "
                        ...
                        {:data-file-or-config/pret-variable-name {:column-name column-name
                                                                  :pret/variables (keys (:pret/variables job))}})))
...
(defn extract-card-many-data
  ...
  when pret directive in job specifies to do so."
  [parsed-cfg schema mapping job raw-entity na-val-set]
  (let [many-att-val-pairs (filter (fn [[k v]]
    ...
    (when (seq many-att-val-pairs)
      (->> (for [[attr {:keys [pret/many-delimiter
                               pret/many-variable]}] many-att-val-pairs]
...
(defn extract-constant-data
  ...
  (when-let [constant-data (or (:pret/constants job)
                               (:pret.matrix/constants job))]
...
(defn handle-na
  ...
  (let [na-attrs (->> (filter #(some #{:pret/missing-value} (flatten [(% 1)])) final-entity)
                      (map first)
                      set)
    ...
    (if-not (seq req-attr)
      ...
      (if (and (= req-attr '[*])
               (seq na-attrs))
          (do
            (log/debug "Omitting entity due to any [*] NA values.:" (:pret/annotations final-entity))
            :pret/omit)
          (if-let [missing (seq (filter na-attrs req-attr))]
            (do
              (log/debug "Omitting entity due to NA values for: " missing " from:" (:pret/annotations final-entity))
              :pret/omit)
            entity-wo-na)))))
...
(defn record->entity
  ...
  (if (not= (count header)
    ...
    (let [;; start-time (System/nanoTime)  ; part of per-record perf logging
          ...
          omit-on (:pret/omit-if-na job)
          {:keys [filename line-number]} import-ctx
          annotation {:pret/annotations {:pret.annotation/filename    (text/file->str filename)
                                         :pret.annotation/line-number line-number}}]
      ...
      (try
        (let [molten-data (extract-molten-data parsed-cfg schema mapping job raw-entity na-val-set)
              ...
              precomputed (:pret/precomputed job)
              ...
              ;; Reference-data is decorated with the :pret.import/most-recent
              ...
              import-metadata (if (reference-data-entity? schema base-entity)
                                {:pret.import/most-recent [:import/name import-name]}
                                {})]
          ;; uncomment this and start-time to log per-record nanosec record perf
          ;;(log/debug ::record (:pret/node-kind job) "computed in "
          ;;           (- (System/nanoTime) start-time) " nanosec."
          (-> base-entity
              (collection/remove-keys-by-ns "pret")
              (merge annotation)
